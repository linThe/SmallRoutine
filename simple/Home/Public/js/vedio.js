...
$.ajax({
	...
	success:function(json){
		var da = json;
		length = da.result.length ;
		var rad = parseInt(Math.random()*length) ;
		var src = da.result[rad].data.content.data.playUrl ; 
		var title = da.result[rad].data.content.data.area ;
//		var img = da.result[rad].data.content.data.tags.headerImage ;
//		$(".img_click").attr("src",img) ;
		if(src != null){
			vids.src= src ;
			$("#titleT").text();
		}else{
		...
		vids.onended = function(){
			var floor = vids.src= da.result[parseInt(Math.random()*length)].data.content.data.playUrl;
			if(floor != null){
				vids.src  = floor ;
				$("#titleT").text();
				vids.play() ;
